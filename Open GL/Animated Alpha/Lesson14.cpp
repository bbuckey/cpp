...
GLvoid glPrint2(const char *fmt, ...)					// Custom GL "Print" Routine
{
	float		length=0;								// Used To Find The Length Of The Text
	char		text[256];								// Holds Our String
	va_list		ap;										// Pointer To List Of Arguments
	...
	glTranslatef(-length/2,-1.0f,0.0f);					// Center Our Text On The Screen
...
GLvoid glPrint3(const char *fmt, ...)					// Custom GL "Print" Routine
{
	float		length=0;								// Used To Find The Length Of The Text
	char		text[256];								// Holds Our String
	va_list		ap;										// Pointer To List Of Arguments
...
GLvoid glPrint4(const char *fmt, ...)					// Custom GL "Print" Routine
{
	float		length=0;								// Used To Find The Length Of The Text
	char		text[256];								// Holds Our String
	va_list		ap;										// Pointer To List Of Arguments
	...
	glTranslatef(-length/2,-2.0f,0.0f);					// Center Our Text On The Screen
...
//	glTranslatef(0,1,0); 	
	glPrint("A B C D E F G",0);
	glTranslatef(-3.1,-1.0,0);
	glPrint("H I J K L M N",0);
	glTranslatef(-2.8,-1.0,0.0);
 	glPrint("O P Q R S T",0);
	glTranslatef(-2.5,-1.0,0);
	glPrint("U V W X Y Z",0);
					...
					int			nCmdShow)			
{
	...
	if (!CreateGLWindow("ABC's",640,480,16,fullscreen))
	{
		return 0;									
	}
